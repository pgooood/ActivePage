...
class apTaglist extends module{
	...
	function getMessage(){
		...
		switch($_SESSION['apMess'][$this->getMessSessionName()]){
			case 'delete_ok':
				...
				break;
			case 'delete_fail':
				$mess = 'Ошибка, запись не удалена';
				break;
			case 'update_ok':
				$mess = 'Информация успешно обновлена';
				break;
			case 'update_fail':
				$mess = 'Ошибка обновления информации';
				break;
			case 'add_ok':
				$mess = 'Запись добавлена';
				break;
			case 'add_fail':
				$mess = 'При добавлении произошла ошибка';
				break;
		}
	...
	function getTagList(){
		$xml = new xml('xml/list.xml', $this->getListTagName(), false);
	...
	function onMove(){
		if(($row = $this->getRow()) && ($pos = param('pos')) > 0 && ($tl = $this->getTagList()) && ($e = $tl->get($row)) && $tl->move($e, $pos)
		){
			$tl->getXML()->save();
			return true;
		}
	...
	function onDelete(){
		if(($row = $this->getRow()) && ($tl = $this->getTagList())
		){
			$num = $tl->getNum();
			if(!is_array($row))
				$row = array($row);
	...
	function onUpdate(){
		if(($row = $this->getRow()) && ($form = $this->getForm()) && ($tl = $this->getTagList()) && $tl->get($row)
		){
	...
	function onAdd(){
		if(($form = $this->getForm()) && ($tl = $this->getTagList())
		){
			$num = $tl->getNum();
			$form->save($_REQUEST);
			$tl = $this->getTagList();
	...
	function onEdit(){
		global $_out;
		if(($row = $this->getRow()) && ($form = $this->getForm())
		){
			$form->replaceURI(array('POSITION' => $row));
			$form->load();
