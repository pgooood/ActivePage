...
class mysql{
	...
	function __construct($id = null){
		...
		if($e = $_site->query('/site/mysql/con' . ($id ? '[@id="' . $id . '"]' : ''))->item(0)){
			...
			throw new Exception('MySQL connection not found', EXCEPTION_MYSQL);
	}
	...
	function query($query, $resultmode = MYSQLI_STORE_RESULT){
		...
		if(!$res)
			throw new Exception(mysql_error() . '<br/><code>' . $query . '</code>', EXCEPTION_MYSQL);
		return $res;
	}
	...
	function getNextId($table){
		if($table && ($rs = $this->query("SHOW TABLE STATUS FROM `" . $this->db . "` LIKE '" . $this->getTableName($table) . "'")) && ($row = $rs->fetch_assoc())
		){
	...
	function num($v){
		...
		return strstr($v, '.') ? number_format(floatval($v), 6, '.', '') : intval($v);
	}
